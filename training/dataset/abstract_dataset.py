...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def load_rgb(self, file_path):
        ...
        if not self.lmdb:
        # ... (the rest of the path logic you fixed)
            # Check if the path is NOT already absolute (i.e., it's a relative path from the JSON)
            if not os.path.isabs(file_path):
                # We need to construct the full path: ProjectRoot/datasets/path_from_json
                # We assume self.config['rgb_dir'] is the project root's 'datasets' folder (i.e., './datasets').

                # Use os.path.join for safe, normalized path construction
                # This ensures the path is built correctly regardless of slashes.
                full_path = os.path.join(self.config['rgb_dir'], file_path)
            else:
                # If the path is already absolute (starts with C:/), use the path directly
                full_path = file_path

            # Crucially, convert the path to the system's preferred format for os.path.exists
            file_path = os.path.normpath(full_path)

            # Final verification: Check for file existence with a clearer error message
            assert os.path.exists(file_path), f"File does not exist at normalized path: {file_path}"

            # ... (rest of the load_rgb function)
    ...
    def __getitem__(self, index, no_norm=False):
        ...
        for image_path in image_paths:
            ...
            try:
                ...
                # Log the error but skip the problematic index to prevent infinite recursion
                print(f"Error loading image at index {index}: {e}. Skipping to next index.")

                # --- SAFE RECOVERY LOGIC ---
                # Try the next index. If the next index exceeds the dataset length, restart from 0.
                # Using (index + 1) % len(self.image_list) is the safest method.
                next_index = (index + 1)

                # Prevent potential infinite loop if the error occurs in all files
                if next_index == len(self.image_list):
                    raise RuntimeError("All images failed to load during testing. Check dataset integrity.")
            ...
            if not no_norm:
                ...
                if self.config['with_landmark']:
                    landmarks_trans = torch.from_numpy(landmarks.astype(np.int32))
                if self.config['with_mask']:
